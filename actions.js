...
const Actions = (() => {
  ...
  async function execCreateBook({ title, author, totalChapters, totalPages, folderName }) {
    if (!title) return result('create_book', false, 'Título do livro não informado.');
    const folderId = await resolveFolder(folderName || 'Livros');
    const id = await GabrielDB.Books.create({ title, author, totalChapters, totalPages, folderId });
    // Cria também uma nota de acompanhamento
    await GabrielDB.Notes.create(
      `📚 ${title}`,
      `Livro: ${title}\nAutor: ${author || 'Desconhecido'}\nCapítulos: ${totalChapters || '?'}\nPáginas: ${totalPages || '?'}\n\nIniciado em: ${new Date().toLocaleDateString('pt-BR')}`,
      folderId
    );
    return result('create_book', true, `Livro **"${title}"** adicionado ao tracker de leitura! 📚`, { id, title, author });
  }

  async function execUpdateBookProgress({ title, currentChapter, currentPage }) {
    if (!title) return result('update_book', false, 'Título do livro não informado.');
    const books = await GabrielDB.Books.getAll();
    const book = books.find(b => b.title.toLowerCase().includes(title.toLowerCase()));
    if (!book) return result('update_book', false, `Livro "${title}" não encontrado. Crie-o primeiro.`);
    const updated = await GabrielDB.Books.updateProgress(book.id, { currentChapter, currentPage });
    const progress = book.totalChapters > 0
      ? Math.round(((currentChapter || book.currentChapter) / book.totalChapters) * 100)
      : null;
    const msg = `Progresso de **"${book.title}"** atualizado! ` +
      (currentChapter ? `Capítulo ${currentChapter}${book.totalChapters ? '/' + book.totalChapters : ''}` : '') +
      (progress ? ` — ${progress}% concluído 📖` : '');
    return result('update_book', true, msg, updated);
  }

  async function execListBooks() {
    const books = await GabrielDB.Books.getAll();
    if (!books.length) return result('list_books', true, 'Nenhum livro no tracker ainda. Adicione um! 📚', { books: [] });
    const stats = await GabrielDB.Books.getStats();
    const list = books.slice(0, 8).map(b => {
      const progress = b.totalChapters > 0 ? ` (${Math.round(b.currentChapter/b.totalChapters*100)}%)` : '';
      const icon = { reading:'📖', finished:'✅', wishlist:'📌' }[b.status] || '📚';
      return `${icon} **${b.title}**${b.author ? ' — ' + b.author : ''}${progress}`;
    }).join('\n');
    return result('list_books', true,
      `📚 Sua biblioteca (${stats.total} livros):\n${list}\n\n✅ Finalizados: ${stats.finished} | 📖 Lendo: ${stats.reading}`,
      { books, stats }
    );
  }
  ...
  async function execLogStudy({ subject, duration, notes, reminderTime, folderName }) {
    if (!subject) return result('log_study', false, 'Informe o assunto estudado.');
    if (!duration) return result('log_study', false, 'Informe a duração em minutos.');
    const folderId = await resolveFolder(folderName || 'Estudos');
    await GabrielDB.StudySessions.create({ subject, duration, notes, reminderTime, folderId });
    const stats = await GabrielDB.StudySessions.getStats();
    const hours = Math.floor(duration / 60);
    const mins  = duration % 60;
    const timeStr = hours > 0 ? `${hours}h${mins > 0 ? mins + 'min' : ''}` : `${mins}min`;
    return result('log_study', true,
      `Sessão de estudo registrada! 🎓\n**${subject}** — ${timeStr}\nTotal acumulado: **${stats.totalHours}h** de estudo`,
      { subject, duration, stats }
    );
  }

  async function execScheduleStudy({ subject, date, time, duration, notes, folderName }) {
    if (!subject || !date) return result('schedule_study', false, 'Informe o assunto e a data.');
    const folderId = await resolveFolder(folderName || 'Estudos');
    // Cria evento de lembrete
    await GabrielDB.Events.create({
      title: `📚 Estudar: ${subject}`,
      date, time: time || '08:00',
      description: notes || `Sessão de estudo: ${subject}${duration ? ' — ' + duration + 'min' : ''}`,
      reminder: true, folderId
    });
    // Cria tarefa
    await GabrielDB.Tasks.create(`Estudar: ${subject}`, date, folderId);
    return result('schedule_study', true,
      `Estudo agendado! 📅\n**${subject}** — ${new Date(date + 'T12:00:00').toLocaleDateString('pt-BR')}${time ? ' às ' + time : ''}`,
      { subject, date, time }
    );
  }

  async function execStudyStats() {
    const stats = await GabrielDB.StudySessions.getStats();
    const recent = await GabrielDB.StudySessions.getRecent(3);
    if (stats.totalSessions === 0) return result('study_stats', true, 'Nenhuma sessão de estudo registrada ainda. Comece a estudar! 💪');
    const recentList = recent.map(s => `• ${s.subject}: ${s.duration}min`).join('\n');
    const topSubjects = Object.entries(stats.subjects)
      .sort((a,b) => b[1]-a[1]).slice(0,3)
      .map(([s,m]) => `• ${s}: ${(m/60).toFixed(1)}h`).join('\n');
    return result('study_stats', true,
      `📊 Suas estatísticas de estudo:\n\n⏱️ Total: **${stats.totalHours}h** em ${stats.totalSessions} sessões\n\n🏆 Top matérias:\n${topSubjects}\n\n📅 Recentes:\n${recentList}`,
      stats
    );
  }
